...
IS
  ...
  MEMBER FUNCTION to_string RETURN VARCHAR2 IS
   ...
   MEMBER FUNCTION merge_moving_points (mp1 moving_point, mp2 moving_point) RETURN moving_point IS
      ...
      first_tp tau_tll.d_timepoint_sec;discard1 pls_integer:=0;
   BEGIN
     if (mp1.srid <> mp2.srid) then
      ...
      THEN
         ...
         --case equal timepoints
         if ( last_tp.f_eq(last_tp, first_tp)=1 ) then
           first_tp := mp2.u_tab(mp2.u_tab.first).p.e;--assume p.b<p.e
           discard1:=1;
         end if;
      ...
      THEN
         ...
         --case equal timepoints
         if ( last_tp.f_eq(last_tp, first_tp)=1 ) then
           first_tp := mp1.u_tab(mp1.u_tab.first).p.e;--assume p.b<p.e
           discard1:=1;
         end if;
         gluesegment:=unit_moving_point(tau_tll.d_period_sec(last_tp,first_tp),unit_function(
   ...
   MEMBER FUNCTION f_duration RETURN NUMBER is
    ...
    begin
        ...
        exception when others then
          dbms_output.put_line('f_duration()->error duration of segment:'||i||' of traj:'||self.traj_id);
    end;
...
/*
   ...
   MEMBER FUNCTION at_period_no_lib (per tau_tll.d_period_sec) RETURN moving_point IS
      ...
      rc integer;
   BEGIN
      DBMS_OUTPUT.put_line ('at period no lib for traj:'||self.traj_id||' and period from '||per.b.m_y||'-'||per.b.m_m||'-'||per.b.m_d||'-'||
        per.b.m_h||'-'||per.b.m_min||'-'||FLOOR(per.b.m_sec)||' to '||per.e.m_y||'-'||per.e.m_m||'-'||per.e.m_d||'-'||
        per.e.m_h||'-'||per.e.m_min||'-'||FLOOR(per.e.m_sec));
      ...
      tmp_tpe:=to_timestamp(u_tab (i).p.e.m_y||'-'||u_tab (i).p.e.m_m||'-'||
        ...
        'yyyy-mm-dd hh24:mi:ss');
      -- Euresi tou 1ou tab pou periexei tin dosmeni timepoint start
      ...
      -- Euresi tou teleutaiou tab pou periexei tin dosmeni timepoint end
      ...
      if i is null then
        --new_per.e is either after the end of u_tab
        --or before the begining of u_tab  or equal so
      ...
      tmp_tpb:=to_timestamp(u_tab (first_tab).p.b.m_y||'-'||u_tab (first_tab).p.b.m_m||'-'||
         ...
         'yyyy-mm-dd hh24:mi:ss');
      tmp_tpe:=to_timestamp(u_tab (first_tab).p.e.m_y||'-'||u_tab (first_tab).p.e.m_m||'-'||
         u_tab (first_tab).p.e.m_d||'-'||u_tab (first_tab).p.e.m_h||'-'||
         u_tab (first_tab).p.e.m_min||'-'||FLOOR(u_tab (first_tab).p.e.m_sec),
         'yyyy-mm-dd hh24:mi:ss');

      IF (b_tp>tmp_tpb)and (b_tp<tmp_tpe) THEN
   ...
   rc:=res_point.count;
      ...
      tmp_tpe:=to_timestamp(u_tab (last_tab).p.e.m_y||'-'||u_tab (last_tab).p.e.m_m||'-'||
        ...
        'yyyy-mm-dd hh24:mi:ss');
      tmp_tpb:=to_timestamp(u_tab (last_tab).p.b.m_y||'-'||u_tab (last_tab).p.b.m_m||'-'||
        u_tab (last_tab).p.b.m_d||'-'||u_tab (last_tab).p.b.m_h||'-'||
        u_tab (last_tab).p.b.m_min||'-'||FLOOR(u_tab (last_tab).p.b.m_sec),
        'yyyy-mm-dd hh24:mi:ss');

      -- Teleutaio Tab
      IF (e_tp<tmp_tpe) and (e_tp>tmp_tpb) THEN
...
BEGIN
  ...
  WHILE i IS NOT NULL LOOP
    ...
    tmp_speed:=mdsys.sdo_geom.sdo_distance(
       ...
       0.0005) / tau_tll.d_period_sec(tau_tll.d_timepoint_sec(u_tab(i).p.b.m_y,u_tab(i).p.b.m_m,u_tab(i).p.b.m_d,u_tab(i).p.b.m_h,u_tab(i).p.b.m_min,u_tab(i).p.b.m_sec),u_tab(i).p.e).duration().m_Value;
  ...
  RETURN max_speed;
  exception when others then
    dbms_output.put_line('error on f_max_speed:'||i||' timepoints:'||
      u_tab(i).p.b.m_y||','||u_tab(i).p.b.m_m||','||u_tab(i).p.b.m_d||','||u_tab(i).p.b.m_h||','||u_tab(i).p.b.m_min||','||u_tab(i).p.b.m_sec);
END;
...
END number_of_times_close;
  ...
  begin
    if (complex='no') then
      ...
      for i in self.u_tab.first..self.u_tab.last loop
        disti := distance(u_tab(i).m.xi, u_tab(i).m.yi, u_tab(i).m.xe, u_tab(i).m.ye);
        center.x:=center.x + disti*(u_tab(i).m.xi + u_tab(i).m.xe)/2;
        center.y:=center.y + disti*(u_tab(i).m.yi + u_tab(i).m.ye)/2;
        total_length := total_length + disti;
      end loop;
      if total_length = 0 then
        total_length := 0.00001;
      end if;
      center.x:= center.x/total_length;
      center.y:= center.y/total_length;
    end if;
    return center;
     exception when others then
          dbms_output.put_line('mass_center()->error of segment:'||i||' of traj:'||self.traj_id);
  end mass_center;
  ...
  begin
    center := mass_center();
    --or use oracle's centroid
    --center_geom := sdo_geom.sdo_centroid(sdo_geom.sdo_convexhull(self.route(), 0.05), 0.05);--returns 1/n( S(xi), S(yi) )

    for i in self.u_tab.first..self.u_tab.last loop
      rg:=rg+power(
        mdsys.sdo_geom.sdo_distance(
          --center_geom,
          mdsys.sdo_geometry(2001,self.srid,sdo_point_type(center.x,center.y,0),null,null),
          mdsys.sdo_geometry(2001,self.srid,sdo_point_type(u_tab(i).m.xi,u_tab(i).m.yi,0),null,null),
          0.005),
          2);
    end loop;
    --plus the last point
    rg:=rg+power(
        mdsys.sdo_geom.sdo_distance(
          --center_geom,
          mdsys.sdo_geometry(2001,self.srid,sdo_point_type(center.x,center.y,0),null,null),
          mdsys.sdo_geometry(2001,self.srid,sdo_point_type(u_tab(u_tab.last).m.xe,u_tab(u_tab.last).m.ye,0),null,null),
          0.005),
          2);
    --avg
    rg:=rg/(u_tab.last+1);
   ...
   begin
    ...
    for i in self.u_tab.first..self.u_tab.last loop
      if (self.u_tab(i).m.xi = self.u_tab(i).m.xe) and (self.u_tab(i).m.yi = self.u_tab(i).m.ye) then
        ...
        --visualizer.geom2wkt(sdo_cs.transform(ellipse,srid) , '21-paa_of_taj_2935_ellipse'||i);
        geometry_array.extend;
