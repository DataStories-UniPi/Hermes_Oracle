...
CREATE OR REPLACE type body sem_episode is
  ...
  */
  ...
  tags varchar_ntab;submpoint sub_moving_point;
  ...
  begin
    ...
    tmpnum:=mbbintersection(1);
    tmpnum:=mbbintersection(2);
    tmpnum:=mbbintersection(3);
    for i in 1..3 loop--check negativity
      ...
      if (mbbe1(i)<0) or (mbbe2(i)<0) then --error on coords
        /*select deref(e.tlink) into submpoint from dual;
        dbms_output.put_line(submpoint.o_id||','||submpoint.traj_id||','||submpoint.subtraj_id);
        select deref(self.tlink) into submpoint from dual;
        dbms_output.put_line(submpoint.o_id||','||submpoint.traj_id||','||submpoint.subtraj_id);*/
        raise invalidcoords;
      end if;
    end loop;
    for i in 1..3 loop--calc spatiotemporal distance, 0 if ispoint some episode
      if (maxdist(i) = 0) then--episodes are same points for dimension i
        null;--distth:=distth + 0*weight(i);
      elsif (greatest(mbbe1(i),mbbe2(i)) = 0) then--episodes are different points for dimension i
    ...
    and (e.activity_tag='****') then--gap episode
      ...
      if (indxprefix is not null) then
        stmt:='select upper(t.tag) from '||indxprefix||'_tag t order by t.tag';--should be distinct already
