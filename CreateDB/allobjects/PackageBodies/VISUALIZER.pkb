...
';
 ...
 procedure movingpointtable2wkt(mpoints mp_array, tag varchar2) is
   filename varchar2(80);
   wkt_geometry clob;
   mpoint moving_point;
   geom sdo_geometry;
   wkt_geometry_tmp clob;
 begin
   filename :=tag||'.wkt';
   dbms_lob.createtemporary(wkt_geometry,TRUE);
   dbms_lob.createtemporary(wkt_geometry_tmp,TRUE);
   for j in mpoints.first..mpoints.last loop
      mpoint := mpoints(j);
      geom := mpoint.route();
      wkt_geometry_tmp := sdo_util.to_wktgeometry(geom);
      dbms_lob.append(wkt_geometry, wkt_geometry_tmp);
   end loop;
   dbms_xslprocessor.clob2file(wkt_geometry, 'GML2KML', filename);
 end movingpointtable2wkt;

 procedure episodes2wkt(episodes sem_episode_tab,srid number, tag varchar2) is
   filename varchar2(80);
   wkt_geometry clob;
   geom sdo_geometry;
   wkt_geometry_tmp clob;
 begin
   ...
   dbms_lob.createtemporary(wkt_geometry,TRUE);
   dbms_lob.createtemporary(wkt_geometry_tmp,TRUE);
   for j in episodes.first..episodes.last loop
      geom := episodes(j).mbb.getrectangle(srid);
      wkt_geometry_tmp := sdo_util.to_wktgeometry(geom);
      dbms_lob.append(wkt_geometry, wkt_geometry_tmp);
   end loop;
   dbms_xslprocessor.clob2file(wkt_geometry, 'GML2KML', filename);
...
procedure semtrajectories2wkt(semtrajs sem_trajectory_tab,tag varchar2,visualizetype number)is
  filename varchar2(80);
  wkt_geometry clob;
  geom sdo_geometry;
  wkt_geometry_tmp clob;
...
begin
  ...
  dbms_lob.createtemporary(wkt_geometry,true);
  dbms_lob.createtemporary(wkt_geometry_tmp,true);
  if (visualizetype = 0) then
    for i in semtrajs.first..semtrajs.last loop
      for j in semtrajs(i).episodes.first..semtrajs(i).episodes.last loop
        if (semtrajs(i).episodes(j).defining_tag = 'STOP') then
          geom := semtrajs(i).episodes(j).mbb.getrectangle(semtrajs(i).srid);
          wkt_geometry_tmp := sdo_util.to_wktgeometry(geom);
          dbms_lob.append(wkt_geometry, wkt_geometry_tmp);
        end if;
      end loop;
    end loop;
  elsif (visualizetype = 1) then
    for i in semtrajs.first..semtrajs.last loop
      for j in semtrajs(i).episodes.first..semtrajs(i).episodes.last loop
        if (semtrajs(i).episodes(j).defining_tag = 'STOP') then
          geom := semtrajs(i).episodes(j).mbb.getrectangle(semtrajs(i).srid);
          wkt_geometry_tmp := sdo_util.to_wktgeometry(geom);
          dbms_lob.append(wkt_geometry, wkt_geometry_tmp);
        elsif (semtrajs(i).episodes(j).defining_tag = 'MOVE') then
          select deref(semtrajs(i).episodes(j).tlink) into submpoint from dual;
          geom := submpoint.sub_mpoint.route();
          wkt_geometry_tmp := sdo_util.to_wktgeometry(geom);
          dbms_lob.append(wkt_geometry, wkt_geometry_tmp);
        end if;
      end loop;
    end loop;
  elsif (visualizetype = 2) then
    for i in semtrajs.first..semtrajs.last loop
      for j in semtrajs(i).episodes.first..semtrajs(i).episodes.last loop
        geom := semtrajs(i).episodes(j).mbb.getrectangle(semtrajs(i).srid);
        wkt_geometry_tmp := sdo_util.to_wktgeometry(geom);
