...
order by t.o_id, t.semtraj_id';
  ...
  WHEN insemtrajs.count=0;
    ...
    LOOP
      ...
      insemtrajs(indx).o_id, insemtrajs(indx).semtraj_id);
      episodecounter:=0;
...
order by t.object_id, t.traj_id';
  ...
  WHEN intrajs.count=0;
    ...
    LOOP
      ...
      intrajs(indx).object_id, intrajs(indx).mpoint.traj_id);
      --episodes none
      episodecounter:=0;
  ...
  loop
    ...
    for i in feathers.first..feathers.last loop
      ...
      if (current_person=previous_person) then
        ...
        current_semtrajid:= 1;
        cursemtraj:=sem_trajectory( null, srid, sem_episode_tab(),current_person, current_semtrajid);--empty
...
procedure discoverepisodes(inputtblstopseqs    varchar2,inputtblmpoints     varchar2,
    ...
    */
    ...
    subtraj_id integer;
    sem_traj   sem_trajectory := sem_trajectory(null,-1,sem_episode_tab(),-1,-1);
          ...
          if (sem_traj.episodes.count > 1) then
            ...
            */
            execute immediate 'insert into ' || outputtblsmpoints ||
              ...
              sem_traj.episodes, sem_traj.o_id,sem_traj.semtraj_id);
            commit;
          end if;
...
BEGIN
  ...
  begin
    ...
    loop
      ...
      for i in sem_trajs.first..sem_trajs.count loop 
        ...
        outsemtrajs.extend(1);
        outsemtrajs(outsemtrajs.last):=sem_trajectory(sem_trajs(i).sem_trajectory_tag,sem_trajs(i).srid,sem_epis_tab,sem_trajs(i).o_id,sem_trajs(i).semtraj_id);        
      end loop;
    end loop;
    close sem_trajs_cur;
  ...
  begin
    ...
    for c in (select * from ATTIKI_SUB_mpoints bs --change table
      ...
      commit;
      update ATTIKI_SUB_mpoints s --change table
        set s.sub_mpoint=(
